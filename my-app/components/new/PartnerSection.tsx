...
export const PartnerSection: React.FC = () => {
  ...
  const PartnerCard = ({ partner, isDuplicate }: { partner: typeof partners[0], isDuplicate: boolean }) => {
      ...
      <div className="relative group/scroll">
        ...
        <div className="flex animate-scroll-rtl hover:[animation-play-state:paused]">
          ...
          {partners.map((partner) => (
            <PartnerCard key={`first-${partner.id}`} partner={partner} isDuplicate={false} />
          ))}
          {/* Duplicate set for seamless looping */}
          {partners.map((partner) => (
            <PartnerCard key={`second-${partner.id}`} partner={partner} isDuplicate={true} />
          ))}
        </div>
      </div>
    </section>
